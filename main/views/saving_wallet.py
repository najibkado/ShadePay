...
def saving_wallet(request):
    ...
    return render(request, "main/saving_wallet.html", {
        "address": wallet.address,
        "name": loggedin_user.first_name + " " + loggedin_user.last_name,
        "balance": wallet.balance,
        "date": wallet.date_due if wallet.is_active else "Not Set",
        "is_active": "Activated" if wallet.is_active else "Not Activated",
        "status": status
    })